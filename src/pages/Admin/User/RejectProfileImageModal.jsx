...
export default function RejectProfileImageModal({ modalOpen, data, closeModal, onSuccess, noSettings }) {
  ...
  async function onSubmit(e) {
    ...
    sdk.setTable("user");
    try {
      await sdk.callRestAPI({ id: data.id, is_photo_approved: IMAGE_STATUS.NOT_APPROVED }, "PUT");

      let settings = data.settings;
      if (noSettings) {
        const userData = await callCustomAPI("get-user", "post", { id: data.id }, "");
        settings = userData.settings;
      }

      if (parseJsonSafely(settings, {}).email_on_profile_photo_declined == true) {
        const tmpl = await sdk.getEmailTemplate("profile-image-decline");
