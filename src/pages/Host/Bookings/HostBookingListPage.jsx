...
import HostBookingFiltersModal from "./HostBookingFiltersModal";

const sdk = new MkdSDK();
const ctrl = new AbortController();

export default function HostBookingListPage() {
  ...
  async function fetchMyBookings(page) {
    ...
    var where = [`ergo_booking.host_id = ${user_id}`];
    ...
    if (filters.id) {
      where = [`ergo_booking.host_id = ${user_id} AND ergo_booking.id = ${filters.id}`];
    }
  ...
  async function fetchFavoriteStatuses() {
    ...
    try {
      const result = await sdk.callRestAPI({ payload }, "GETALL", ctrl.signal);
      if (Array.isArray(result.list)) {
