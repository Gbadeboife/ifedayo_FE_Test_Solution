...
import { BOOKING_STATUS, NOTIFICATION_STATUS, NOTIFICATION_TYPE } from "@/utils/constants";
import { LoadingButton } from "@/components/frontend";
import { usePropertySpace, usePublicUserData } from "@/hooks/api";
...
export default function HostBookingDetailsPage() {
  ...
  const otherUserData = usePublicUserData(booking.customer_id);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const result = await callCustomAPI("review", "post", newReview, "POST");
  ...
  async function fetchBooking(booking_id) {
    ...
    const where = [`ergo_booking.id = ${booking_id}`];
    try {
    ...
    <div className="text-sm normal-case md:text-base">
      ...
      <div className="mb-[22px] flex flex-col justify-between md:flex-row">
        ...
        <div className="flex items-center gap-[16px]">
          ...
          <div className="flex flex-grow justify-end">
            <div className={`${!bookingExpired ? "block" : "hidden"} "border p-1 px-2 md:border-0"`}>
              <ThreeDotsMenu
                items={[
                  {
                    ...
                    notShow: booking.status == BOOKING_STATUS.COMPLETED || booking.status == BOOKING_STATUS.PENDING || booking.status == BOOKING_STATUS.CANCELLED || bookingExpired,
                  },
                  {
                    label: "Accept booking",
                    icon: <></>,
                    onClick: () => setAcceptPopup(true),
                    notShow: booking.status !== BOOKING_STATUS.PENDING || bookingExpired,
                  },
                  {
      ...
      <div className="flex flex-wrap items-start md:gap-[24px]">
        <div className="w-full md:w-[55%]">
              ...
              <div>
                ...
                <p className="text-[#667085]">
                  ...
                  <Link
                    ...
                    to={`/account/messages?other_user_id=${booking.customer_id}&booking=${booking.id}&space=${booking.property_space_id}`}
                  >
                    Messages
                  </Link>{" "}
                </p>
          ...
          <div className="mb-32 flex flex-wrap gap-x-24 gap-y-4">
        ...
        <div className="max-w-full flex-grow">
          <div className="mb-[32px] rounded-lg border border-[#EAECF0] bg-[#F9FAFB] px-[20px] py-[24px]">
            <h3 className="mb-[8px] font-semibold md:text-2xl">Your Guest</h3>
            <div className="flex flex-wrap items-center justify-between">
              <div className="mb-4 flex items-center gap-4">
                ...
                />
                <p className="font-semibold md:text-xl">{(booking.customer_first_name ?? "") + " " + (booking.customer_last_name ?? "")}</p>
              </div>
              <Link
          ...
          {state.role == "customer" &&
            <div className="mb-[16px] rounded-lg border border-[#EAECF0] bg-[#F9FAFB] p-[20px] md:p-[32px]">
              ...
              <p className="mb-[16px] text-sm text-[#667085]">(You will not be charged until the host accepts your booking)</p>
      ...
      {showAddReviewPopup && (
        <div
          className="popup-container flex items-center justify-center normal-case twentyone-step"
        ...
        >
          ...
          >
            ...
            <p className="mb-[16px] text-sm text-[#475467]">Leave a review to let others know about your experience with the space and the customer.</p>
            <div>
